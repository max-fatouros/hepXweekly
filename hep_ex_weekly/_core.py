...
print("🔎 Optional: Enter keywords to filter papers (e.g. CMS, ATLAS, FCC, ML, GNN). Press Enter to skip.")
keyword_input = input("Filter by keywords (comma-separated): ").strip()
keywords = [k.strip() for k in keyword_input.split(",") if k.strip()] if keyword_input else []
...
print(f"\n📚 Found {len(papers)} hep-ex papers published in the last {DAYS_BACK} days:\n")

for i, p in enumerate(papers, 1):
    print(f"{i}. {p['title']}")
    print(f"   👨‍🔬 {p['authors']}")
    print(f"   🗓️ {p['date']} — arXiv:{p['id']}  📌 {p['announce_type']}  🧠 {p['summary'][:200]}...")
    print(f"   🔗 {p['pdf']}\n")
